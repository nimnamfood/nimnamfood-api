...
public class UpdateIngredientCommandHandler implements CommandHandler<UpdateIngredientCommand, Void> {
    ...
    public Tuple<Void, List<DomainEvent>> execute(UpdateIngredientCommand command) {
        ...
        final Tuple<Ingredient, IngredientChanged> tuple = ingredient.get().updated(
                command.name, command.unit);
        Repositories.ingredients().update(tuple._1);

        return tuple.map(((updatedIngredient, event) -> Tuple.of(null, List.of(event))));
    }
}
